    ...
    <div class="fixed inset-0 bg-background/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div class="bg-background rounded-xl p-6 w-full max-w-lg shadow-lg">
        <div class="flex justify-between items-center mb-4">
          ...
          <Button variant="ghost" on:click={closeModal} size="icon" class="text-muted-foreground bg-background  hover:text-foreground rounded-full">
            <X size={24} />
          </Button>
        </div>
        ...
        <form on:submit|preventDefault={handleSubmit} class="space-y-4">
          ...
          {#if mediaPreviewUrl}
            <div class="relative">
              ...
              {/if}
              <Button variant="ghost" on:click={removeSelectedMedia} size="icon" class="text-muted-foreground bg-background  hover:text-foreground rounded-full absolute top-2 right-2 p-1">
                <X/>
              </Button>
            </div>
          {:else}
