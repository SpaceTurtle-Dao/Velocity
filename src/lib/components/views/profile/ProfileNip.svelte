  ...
  const postEvents: Event[] = [
    {
      ...
      kind: 1,
      tags: [[]],
      content: "Just created a new cryptocurrency called Bitcoin. What do you think?"
    },
    {
      id: "post_event_id_2",
      pubkey: "user_public_key",
...
<div class="max-w-4xl mx-auto p-4">
  <Card class="mb-10 overflow-hidden transition-transform transform hover:scale-105 duration-300 shadow-lg rounded-lg">
    <div class="bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 p-8">
      <div class="flex items-center space-x-6">
        ...
        <div>
          ...
          <p class="text-lg text-pink-100">@{profileData.name.toLowerCase().replace(/\s/g, '')}</p>
        </div>
      </div>
    </div>

    <CardContent class="bg-white bg-opacity-70 backdrop-filter backdrop-blur-lg p-6 rounded-b-lg">
      <p class="text-gray-700 mb-4">{profileData.about}</p>
      <div class="flex justify-between mt-6 space-x-4">
        <div class="text-center">
          <p in:fly={{ y: 20, duration: 500 }} class="text-3xl font-semibold text-purple-600">
            {followerEvents.length}
          </p>
          <p class="text-gray-600">Followers</p>
        </div>
        <div class="text-center">
          <p in:fly={{ y: 20, duration: 500, delay: 100 }} class="text-3xl font-semibold text-pink-600">
            {followingEvents.length}
          </p>
          <p class="text-gray-600">Following</p>
        </div>
        <div class="text-center">
          <p in:fly={{ y: 20, duration: 500, delay: 200 }} class="text-3xl font-semibold text-red-600">
            $10,000,000
          </p>
...
<div class="grid grid-cols-4 gap-4 mb-6">
  {#each [
    { name: "posts", icon: MessageCircle, label: "Posts" },
    { name: "followers", icon: Users, label: "Followers" },
    { name: "following", icon: UserPlus, label: "Following" },
    { name: "marketCap", icon: BarChart3, label: "Market Cap" }
  ] as tab}
    <Button
      class="relative overflow-hidden transition-all duration-300 {activeTab === tab.name ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-200 hover:bg-gray-600'}"
  ...
  {#if activeTab === "posts"}
    <div class="space-y-4">
      {#each postEvents as post}
        <Card>
        <CardContent class="p-6 bg-gray-800 text-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
      ...
      {#each followerEvents as follower}
        <Card>
          <CardContent class="p-6 bg-gray-800 text-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
            ...
            <p class="text-sm text-gray-400">
              Followed on: {new Date(follower.created_at * 1000).toLocaleString()}
            </p>
          </CardContent>
        </Card>
      {/each}
  ...
  {:else if activeTab === "following"}
    <div class="space-y-4">
      {#each followingEvents as following}
        <Card>
          <CardContent class="p-6 bg-gray-800 text-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
            <p class="text-lg font-medium mb-2">Following: {following.tags[0][1]}</p>
            <p class="text-sm text-gray-400">
              Followed on: {new Date(following.created_at * 1000).toLocaleString()}
            </p>
          </CardContent>
        </Card>
      {/each}
  ...
  {:else if activeTab === "marketCap"}
    <Card class="overflow-hidden transition-all duration-300 hover:shadow-lg">
      <CardHeader>
        <CardTitle class="text-xl font-bold text-secondary-700">Market Cap Over Time</CardTitle>
      </CardHeader>
      <CardContent>
