  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
    {#each memes as meme (meme.Pool)}
      <Link to="/Feed/{meme.Pool}" class="block">
        <Card class="bg-white text-gray-800 rounded-lg overflow-hidden shadow-lg flex flex-col h-full transition-transform duration-200 hover:scale-105 border border-gray-200">
          <CardHeader class="p-3 bg-gradient-to-r from-purple-100 to-indigo-100">
            <div class="flex items-center space-x-3">
              <Avatar class="w-10 h-10">
                ...
                />
                <AvatarFallback class="bg-purple-200 text-purple-700">{meme.Creator.slice(0, 2)}</AvatarFallback>
              </Avatar>
              <div class="overflow-hidden">
                <CardTitle class="text-sm font-semibold truncate text-purple-700">
                  {truncate(meme.Creator, 15)}
                </CardTitle>
                <p class="text-gray-500 text-xs truncate">@{meme.Profile?.Name || meme.Creator}</p>
              </div>
            </div>
          </CardHeader>
          ...
          <CardContent class="p-3 flex-grow overflow-hidden bg-white">
            {#if meme.Post.Kind == "0"}
              <p class="text-sm mb-2 line-clamp-3 text-gray-700">{JSON.parse(meme.Post.Content).content}</p>
              <img 
                alt="Meme content" 
              ...
              />
            {:else}
              <p class="text-sm line-clamp-5 text-gray-700">{meme.Post.Content}</p>
            {/if}
          </CardContent>
          ...
          <CardFooter class="px-3 py-2 bg-gray-100 mt-auto">
            <div class="flex justify-between items-center w-full text-xs text-gray-600">
              <div class="flex items-center space-x-1">
                ...
                <span>{meme.Analytics.MarketCap ? `$${meme.Analytics.MarketCap}` : '$0'}</span>
              </div>
              <div class="flex items-center space-x-1">
                <TrendingUp class="w-4 h-4 text-green-400" />
                <span>{meme.Analytics.Price ? `$${meme.Analytics.Price}` : '$0'}</span>
              </div>
            </div>
          </CardFooter>
